...
if (Test-Path $dbFile) {
    ...
    Write-Host "[✓] Local DB backup created at: $targetBackup" -ForegroundColor Green
}
...
if (Test-Path "X:\") {
    ...
    $xZip = "X:\Development_Backups\researchos_full_archive_$timestamp.zip"
    ...
    Write-Host "[✓] Mirrored to: $xDirect" -ForegroundColor Green
    Write-Host "[✓] Snapshot to: $xBackup" -ForegroundColor Green
} else {
    Write-Host "[!] Drive X: not detected; skipping external drive backup." -ForegroundColor Yellow
}
